...
$.fn.extend({
	...
	tabsBind:function(options){
		...
		var r=[];
		this.each(function(){
			if($(this).hasClass("zt_tabs_binded"))return true;
			$(this).addClass("zt_tabs_binded");
			var T=$(this);
	...
	tabLayerBind:function(options){
		...
		var r=[];
		this.each(function(){
			if($(this).hasClass("zt_tabLayer_binded"))return true;
			$(this).addClass("zt_tabLayer_binded");
			var tabs=$(this).find(settings.tabs);
			if(settings.boxs){
...
$(function(){
   function bc_scroll(id,cur){
	...
	kefu() //在线客服 20140910 jiadangwei@baicheng.com
    function sidebar(){
		...
		sideli.hover(function(){
			$(this).find(".si-layer").show();
		},function(){
			$(this).find(".si-layer").hide();
		});
   ...
   //气泡提示
       function header(){
		$(".zq_header").each(function(index, element) {
        var T=$(this);
		var Hover=T.find(".hover");
		var Layer=T.find(".s-layer-z");
		var shijian;
